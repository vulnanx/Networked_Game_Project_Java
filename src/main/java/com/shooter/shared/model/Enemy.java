...
public class Enemy implements Serializable {
     ...
     *
     * TODO (Member B):
     *   1. Calculate dx = targetX - (x + width/2), dy = targetY - (y + height/2)
     *   2. Normalize the vector: length = sqrt(dx*dx + dy*dy)
     *   3. If length > 0: x += (dx/length) * speed, y += (dy/length) * speed
     *   4. For RANGED type: stop moving when within Constants.RANGED_SHOOT_RANGE
     */
    public void moveToward(float targetX, float targetY) {
        // TODO: implement normalized movement toward target
    }
